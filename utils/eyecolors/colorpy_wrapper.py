...
__all__ = ["make_color_swatch", "plot_spectrum", "plot_multipanel_ispec"]
...
def plot_multipanel_ispec(files, plotdir="../../figures/", savetag="multi_ispec_test",
    ...
    for i in [0,1,2,3,4,5]:
        ...
        # Readfile
        dat = np.genfromtxt(files[i]["file"], skip_header=files[i]["header"])
        wl, flx = dat[:,files[i]["iwlflx"][0]], dat[:,files[i]["iwlflx"][1]]

        # Make arrays smaller for speed
        wmask = (wl > 0.2) & (wl < 1.0)
        wl = wl[wmask]
        flx = flx[wmask]
        ...
        # Get delta wavelengths
        dwlnm = np.fabs(wlnm[:-1] - wlnm[1:])
        dwlnm = np.hstack([dwlnm,dwlnm[-1]])

        # Convert to flux
        spec = flx * np.pi * dwlnm * 1e-3

        # Normalize flux
        spec = spec / np.max(spec[(wl > lammin) & (wl < lammax)])

        # Plot
        plot_spectrum(wlnm, spec, title="", show_cie=False, ytitle=r"Relative Intensity",
                     wlmin=lammin*1e3, wlmax=lammax*1e3, ax=ax[i])

        ax[i].set_ylim(0,1)
        ax[i].text(0.5, 0.98, files[i]["name"],\
